...
func cache_gear(player: Player) -> void:
	...
	cache_item(
		player.user_interface.inventory.get_weapon(), weapon_node
	)
	cache_item(
		player.user_interface.inventory.get_shield(), shield_node
	)
	cache_item(
		player.user_interface.inventory.get_armor(), armor_node
	)
